...
async function running_rwa() {
    ...
    let result = await callService(
      `http://37.32.27.143:5020/api/v2.0.0/algorithms/rwa/start?project_id=${project_id}&grooming_id=${grooming_id}`,
      requestOptions
    );
  ...
  let funcA = async () => {
    ...
    if (finished) {
      ...
      let rwa_result = await callService(
        `http://37.32.27.143:5020/api/v2.0.0/algorithms/rwa/check`,
        requestOptions
      );
      let div = document.getElementById("myBar");
