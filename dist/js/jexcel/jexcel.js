                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio' || obj.options.columns[x].type == 'hidden') {
                        ...
                        if (!isNaN(value)) {
                            value = Number(value);
                            let element;
                            if (cell.getAttribute("id").includes("tm")) {
                                if ((element = document.getElementById(cell.getAttribute("id") + "_a")) !== null) {
                                    element.remove();
                                    cell.style.backgroundColor = 'white';
                                }
                            } else if (cell.getAttribute("id").includes("node")) {
                                if ((element = document.getElementById(cell.getAttribute("id") + "_a")) !== null) {
                                    element.remove();
                                    cell.style.backgroundColor = 'white';
                                }
                        ...
                        } else if ($(`#${cell.getAttribute("id")}_a`).length == 0) {
                            let newA = document.createElement('a');
                            // let cellName = jexcel.getColumnNameFromId([cell.dataset.x, cell.dataset.y]);
                            newA.href = "#" + cell.getAttribute("id");
                            newA.setAttribute("id", cell.getAttribute("id") + "_a")
                            newA.setAttribute("onclick", "clickOnError(this,event)")
                            let newspan = document.createElement('span');
                            newspan.innerHTML = "value must be float or integer";
                    ...
                    } else if (obj.options.columns[x].type == 'integer') {
                        var value = cell.children[0].value;
                        if (value.substr(0, 1) != '=') {
                            if (value == '') {
                                value = obj.options.columns[x].allowEmpty ? '' : 0;
                        ...
                        if (Number.isInteger(Number(value))) {
                            ...
                            newA.setAttribute("onclick", "clickOnError(this,event)")
                            let newspan = document.createElement('span');
                            newspan.innerHTML = "value must be integer";
                            newA.appendChild(newspan);
                            let newLine;
                            newLine = document.createElement('br');
                            if (cell.getAttribute("id").includes("tm")) {
                        ...
                        cell.children[0].onblur = null;
                    } else if (obj.options.columns[x].type == 'float') {
                        var value = cell.children[0].value;
                        if (value.substr(0, 1) != '=') {
                            if (value == '') {
                                value = obj.options.columns[x].allowEmpty ? '' : 0;
                        ...
                        if ((/^[+]?([0-9]{1,2})*[.]([0-9]{1,10})?$/.test(value))) {
                            ...
                            newA.setAttribute("onclick", "clickOnError(this,event)")
                            let newspan = document.createElement('span');
                            newspan.innerHTML = "value must be float";
                            newA.appendChild(newspan);
                            let newLine;
                            newLine = document.createElement('br');
                            if (cell.getAttribute("id").includes("tm")) {
