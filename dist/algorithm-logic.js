function groomingAutomationStart(projectId, data) {
    ...
    const request = {
        url: serverAddr + "algorithms/grooming/start/automatic?project_id=" + projectId,
        method: 'POST',
...
async function getAllGrooming() {
    ...
    if (project == undefined || (project[0].id == 0)) {
        toastr.error("you must save to server/load project before load grooming list")
        return
    }
    let loadQuery = {
        project_id: {
                ...
                if (groomingIds.length == 0) {
                    ...
                    for (let i = 0; i < response.body.length; i++) {
                        ...
                        $("#grooming-list-table>tbody").append(
                            "<tr>" +
                            "<td>   <input  type=\"radio\" name=\"radioSelected\" value=\"" + data.id + "\" onclick=\"GroomingSelected('" + data.id + "')\"> </td>\n" +
                            "                        <td>" + data.pt_version + "</td>\n" +
                            "                        <td>" + data.tm_version + "</td>\n" +
                            "                        <td>" + data.start_date + "</td>\n" +
                            "                        <td>" + data.end_date + "</td>\n" +
                            "                        <td>" + data.with_clustering + "</td>\n" +

                            // "                        <td>" + groomingStateData.state + "</td>\n" +
                            "<td><select onchange=\"showData(this);\" id=\"" + data.id + "\" class=\"form-select\" aria-label=\"Default select example\"><option selected>Select cluster</option> </select></td>" +
                            "                        <td>" + data.form.mp1h_threshold + "</td>\n" +
                            "                        <td>" + data.form.comment + "</td>\n" +
                            "                        <td>" + data.algorithm + "</td>\n" +
                            "                        <td>\n" +
                            "                            <button type=\"button\" class=\"btn btn-success\" title=\"View Grooming\" onclick=\"viewGrooming(this, '" + data.id + "')\" ><i class=\"far fa-eye\"></i></button>\n" +
                            "                            <button type=\"button\" class=\"btn btn-info Statistical\" title=\"Statistical Result\" data-toggle=\"popover\" data-placement=\"right\" data-content=\"[groomout_no: " + Statisctical[0] + " | [lightpath_no: " + Statisctical[1] + "]  |  [mean_lightpath_cap: " + Statisctical[2] + "]  |  [mp1h_no: " + Statisctical[3] + "]  |  [mp2x_no: " + Statisctical[4] + "]  |  [tp1h_no: " + Statisctical[5] + "]\"><i class=\"fas fa-poll\"></i></button>\n" +
                            "                            <button id=\"view" + data.id + "\" type=\"button\" class=\"btn btn-primary\" title=\"Grooming Table\"  onclick=\"viewgroomingTable('" + data.id + "')\"><i class=\"fas fa-border-all\"></i></button>\n" +
                            "                        </td>" +
                            "</tr>");
                        ...
                        for (j = 0; j < CluserID.length; j++) {
                            if (CluserID.length > 0) {
                                ...
                                options = "<option value=" + item + "  id=" + item + " class=" + clusterName + " >" + clusterName + "</option>";
...
async function getAllClustring() {
    ...
    result = await callService(`http://185.211.88.140:80/api/v2.0.0/clustering/manual/read_all?project_id=${project_id}`, requestOptions);
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        $("#clustring-list-table>tbody").append("<tr id=" + result[i].id + ">" +
            "                        <th>" + result[i].name + "</th>\n" +
            "                        <td>" + result[i].data.gateways + "</td>\n" +
            "                        <td>" + result[i].data.subnodes + "</td>\n" +
            "                        <td>" + result[i].data.color + "</td>\n" +
            "                        <td>\n" +
            "                            <button type=\"button\" class=\"btn btn-danger\" onclick=\'deleteClustring(" + res + ")\'><i class=\"far fa-trash-alt\"></i></button>\n" +
    ...
    try {
        await SwaggerClient.http(request).then(async response => {
            if (response.status === 200) {
                ...
                if (groomingIds.length == 0) {
                    $("<span>There is not any RWA for this project</span>").insertAfter($("#rwa-list-table"));
                ...
                for (let data of response.body) {
                    ...
                    console.log('groomingStateData is : ', groomingStateData, data, data.pt_version, data.tm_version, data.start_date, data.end_date)
                    ...
                    // groomingState.find()
                    $("#rwa-list-table>tbody").append("<tr>" +
                        "<td>" + i + "</td>\n" +
                        "                        <td>" + data.pt_version + "</td>\n" +
                        "                        <td>" + data.tm_version + "</td>\n" +
                        "                        <td>" + data.start_date + "</td>\n" +
                        "                        <td>" + data.end_date + "</td>\n" +
                        "                        <td>" + data.form.algorithm + "</td>\n" +
                        // "                        <td>" + data.with_clustering + "</td>\n" +
                        "                        <td>" + groomingStateData.state + "</td>\n" +
                        "                        <td>" + groomingStateData.current + "</td>\n" +
                        "                        <td>" + groomingStateData.total + "</td>\n" +
                        "                        <td>" + data.form.comment + "</td>\n" +
                        "                        <td>\n" +
                        "                            <button type=\"button\" class=\"btn btn-success\" title=\"viewGrooming\" onclick=\"viewGrooming(this, '" + data.grooming_id + "')\"><i class=\"far fa-eye\"></i></button> \n" +
                        "                            <button type=\"button\" class=\"btn btn-info\" title=\"Show lightPath\"  id=\"show-light-path\" onclick=\"getLightPathList('" + data.id + "')\"><i class=\"fa fa-list\"></i></button>\n" +
                        "                            <button type=\"button\" class=\"btn btn-secondary \" title=\"RWA General Info\" data-toggle=\"popover\" data-placement=\"left\" data-content=\"Protection: [average_lambda_capacity_usage: " + source[0] + " | [total_lambda_link: " + source[1] + "]  Working:  [average_lambda_capacity_usage: " + source[2] + "]  |  [total_lambda_link: " + source[3] + "]\"><i class=\"fas fa-poll\"></i></button>\n" +
                        "                            <button type=\"button\" class=\"btn btn-warning\" title=\"Lom_Excel\" id=\"downloadlink\" onClick=\" Call_Lom_Excel('" + RWA_Id + "')\"><i class=\"fa fa-download\"></i></button> \n" +
...
async function getRWA_general_info(rwa_id) {
    ...
    if (info.protection) {
        protection_average_lambda = info.protection.average_lambda_capacity_usage.toFixed(5);
    ...
    if (info.working) {
        working_average_lambda = info.working.average_lambda_capacity_usage.toFixed(5);
...
async function getLightPathList(RWA_Id) {
    ...
    let lightPaths;
    let data_result = callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/rwa/result?rwa_id=${RWA_Id}`, requestOptions);
    ...
    data_result.then((value) => {
        ...
        $(".lighpath_table .modal-header").append("<h5 class=\"modal-title\">LightPath List</h5><button type=\"button\" class=\"btn btnClose\">&times;</button>");
        ...
        for (var lightPathId in lightPaths) {
            ...
            $(".lighpath_table #modal_lighpath_table #rwa-lightpath-list").append("<div class='card' id=" + lightPathId + ">" +
                "<div class='card-header selected' id=ss" + lightPathId + ">" +
                " <h2 class='mb-0'>" +
                " <button class='btn btn-link btn-block text-left' type='button' data-toggle='collapse' data-target='#collapse" + lightPathId + "' aria-expanded='true' aria-controls='collapse" + lightPathId + "'>" + "source: " + source + " | " + "destination: " + destination + " | " + "capacity: " + capacity + " | " + "protectionType: " + protection_type +
                "</button>" +
                "</h2>" +
                "</div>" +
                "<div id='collapse" + lightPathId + "' class='collapse' aria-labelledby=" + lightPathId + " data-parent='#rwa-lightpath-list'>" +
                "<div class='card-body'>" +
                "<table class='table'>" +
                "<thead>" +
                "<tr>" +
                ...
                "<tr>" +
                "<th>" + "Protection" + "</th>" +
                "<td>" + protection_snr + "</td>" +
                "<td>" + protection_wavelength + "</td>" +
                "<td>" + protection_regenerators + "</td>" +
                "<td>" + protection_lambda_link + "</td>" +
                "</tr>" +
                "<tr>" +
                "<th>" + "Working" + "</th>" +
                "<td>" + working_snr + "</td>" +
                "<td>" + working_wavelength + "</td>" +
                "<td>" + working_regenerators + "</td>" +
                "<td>" + working_lambda_link + "</td>" +
                "</tr>" +
                "</tbody>" +
                "</table>" +
                "</div>" +
...
async function viewGrooming(element, groomingId) {
    ...
    window.open("http://" + window.location.hostname + ":5000/" + project[0].name + "&1&" + groomingId, '_blank').focus();
}
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                fetch(`http://185.211.88.140:80/api/v2.0.0/clustering/?cluster_id=${ClusterId}`, requestOptions)
                    .then(response => response.text())
                    .then(result => console.log(result))
                    .catch(error => console.log('error', error));
                ...
                request.onsuccess = function (event) {
                    ...
                    request.onsuccess = function (event) {
                        ...
                        updateObject = event.target.result.clusters.filter(row => row.id != ClusterId);
...
function getLatLng(nodeName) {
    ...
    let selectedRow = physical_topologies.nodes.filter(row => row.name == nodeName);
...
async function startEnd2EndGrooming(){
    ...
    result = await callService(`http://185.211.88.140:80/api/v2.0.1/algorithms/automatic/end_to_end/read_all?project_id=${project_id}`, requestOptions)
...
function SubmitGroomingEndToEnd(project_id) {
    ...
    callService(`http://185.211.88.140:80/api/v2.0.1/algorithms/grooming/automatic/end_to_end?project_id=${project_id}`, requestOptions);
...
function SubmitGroomingAdvanced(project_id) {
    ...
    let mp1h_threshold = document.getElementById("grooming-mp1h-threshold").value;
    let grooming_lineRate_threshold = document.getElementById("grooming-lineRate-threshold").value;
    let grooming_comment = document.getElementById("grooming-comment").value;

    let clusters_id_Array = [];

    //Save values of checkbox
    const checkboxes = document.querySelectorAll('input[name="chkClusters"]:checked');
    checkboxes.forEach((checkbox) => {
        clusters_id_Array.push(checkbox.value);
    });

    console.log("clusters_id_Array", clusters_id_Array, mp1h_threshold, grooming_lineRate_threshold, grooming_comment);
    //Call Service
    var myHeaders = new Headers();
    myHeaders.append("Authorization", `${userData.token_type} ${userData.access_token}`);
    myHeaders.append("Content-Type", "application/json");
    ...
    callService(`http://185.211.88.140:80/api/v2.0.1/algorithms/grooming/automatic/advanced?project_id=${project_id}`, requestOptions);
...
async function getStatisticalResult(groomingId) {
    ...
    result = await callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/grooming/result?grooming_id=${groomingId}`, requestOptions);
...
async function viewgroomingTable(groomingId) {
    ...
    $(".lighpath_table .modal-header").html("");
    $(".lighpath_table .modal-header").append("<h5 class=\"modal-title\">Grooming Table</h5><button type=\"button\" class=\"btn btnClose\">&times;</button>");
    $(".lighpath_table #modal_lighpath_table #rwa-lightpath-list ").html("");
    let CountRow = 0;
    var selectedItem;
    for (var ItemId in Table) {
        ...
        $(".lighpath_table #modal_lighpath_table #rwa-lightpath-list").append("<div class='card' id=" + ItemId + " onclick=\"viewSourceDestination('" + source + "','" + destination + "')\">" +
            "<div class='card-header selected' id=ss" + ItemId + ">" +
            " <h2 class='mb-0'>" +
            " <button  id=btn" + ItemId + " class='btn btn-link btn-block text-left' type='button' data-toggle='collapse' data-target='#collapse" + ItemId + "' aria-expanded='true' aria-controls='collapse" + ItemId + "'>" + "source: " + source + " | " + "destination: " + destination +
            "</button>" +
            "</h2>" +
            "</div>" +
            "<div id='collapse" + ItemId + "' class='collapse' aria-labelledby=" + ItemId + " data-parent='#rwa-lightpath-list'>" +
            "<div class='card-body'>" +
            "<table  class='table' id=Table" + ItemId + ">" +
            "<thead>" +
            "<tr>" +
            "<th>" + "#" + "</th>" +
            "<th>" + "Source" + "</th>" +
        ...
        if (splitted_sections.length > 0) {
            for (let k = 0; k < splitted_sections.length; k++) {
                ...
                $(".lighpath_table #modal_lighpath_table #rwa-lightpath-list #Table" + ItemId + " tbody").append("<tr class=\"Splitted\">" + "<th>" + "Splitted Sections" + "</th>" +
                    "<td>" + source_Splitted + "</td>" +
                    "<td>" + destination_Splitted + "</td>" +
                    "<td>" + Type_Splitted + "</td>" +
                    "<td>" + Count_Splitted + "</td>" + "</tr>");
            }
        }
        ...
        if (end_to_ends.length > 0) {
            for (let j = 0; j < end_to_ends.length; j++) {
                ...
                $(".lighpath_table #modal_lighpath_table #rwa-lightpath-list #Table" + ItemId + " tbody").append("<tr class=\"End\">" + "<th>" + "End to Ends" + "</th>" +
                    "<td></td>" +
                    "<td></td>" +
                    "<td>" + Type_endToend + "</td>" +
                    "<td>" + Count_endToend + "</td>" + "</tr>")
            }
        }
    }
...
async function export_grooming_results() {
    ...
    if (grooming_id == undefined || grooming_id == null) {
        ...
        alert("لطفا از قسمت Grooming options >> Grooming List یک Grooming را انتخاب کنید.")
    }
    else {
        var myHeaders = new Headers();
        myHeaders.append("Authorization", `${userData.token_type} ${userData.access_token}`);
        ...
        result = await callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/grooming/result?grooming_id=${grooming_id}`, requestOptions);
        // console.log(result)
        download("grooming_result_" + new Date().toLocaleString() + ".json", JSON.stringify(result, undefined, 2))
    }
...
async function export_full_details_xlsx() {
    ...
    if (grooming_id == undefined || grooming_id == null) {
        ...
        grooming_result = await callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/grooming/result?grooming_id=${grooming_id}`, requestOptions);
        // console.log(result)
    ...
    if (rwa_id == undefined || rwa_id == null) {
        ...
        // result = await callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/rwa/result?rwa_id=b48a8220-a594-40ce-b83d-60795b3ca046`, requestOptions);
        rwa_result = await callService(`http://185.211.88.140:80/api/v2.0.0/algorithms/rwa/result?rwa_id=${rwa_id}`, requestOptions);
    }
